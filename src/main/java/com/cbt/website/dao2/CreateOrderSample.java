...
import fpx.api.orderonline.entity.CreateOrderRequest;
import fpx.api.orderonline.entity.CreateOrderResponse;
...
/**
 * @author Terence chen 2012-6-29 下午2:31:50
 * @version 1.0
 */
public class CreateOrderSample {
	//创建订单API
	public String createOrderService(UserOrderDetails uod) {
		//封装订单信息
		CreateOrderRequest objRequest = getCreateOrderRequest(uod);
		//创建订单支持批量操作,这里只创建单个订单
		CreateOrderRequest[] aobjRequest = new CreateOrderRequest[1];
		aobjRequest[0] = objRequest;
		//创建4PX 提供的SDK调用方式代理类 - 在线订单操作代理类
		OrderOnlineDelegate objDelegate = new OrderOnlineDelegate();
		//远程调用创建订单API
		CreateOrderResponse[] aobjResponse = null;
		try {
			aobjResponse = objDelegate.createOrderService(IOrderOnlineSampleBasic.PRODUCT_AUTHTOKEN, aobjRequest);
	...
	//封装订单信息
	public CreateOrderRequest getCreateOrderRequest(UserOrderDetails uod) {
		CreateOrderRequest objRequest = new CreateOrderRequest();
		// 客户订单号码，由客户自己定义(length <=20)
		objRequest.setOrderNo(uod.getOrderno());
		// 服务商跟踪号码，由4PX提供，客户如果有相应服务商跟踪号，也可以提供，但我司会校验是否有效，无效则重新分配(Length < =30)
		String randomNum = (Integer.toHexString(new Random().nextInt())).substring(0, 2);
		objRequest.setTrackingNumber(uod.getOrderno()+randomNum);// 生成2位随机数放到订单号后面作为trackingNumber
...
//			objDeclareInvoice.setHsCode("TTT");  //编号
	...
	//打印返回结果
	public void printResponse(CreateOrderResponse[] aobjResponse) {
		if (aobjResponse != null && aobjResponse.length > 0) {
			for (int i = 0, iLength = aobjResponse.length; i < iLength; i++) {
				CreateOrderResponse objResponse = aobjResponse[i];
				// 服务器响应时间
