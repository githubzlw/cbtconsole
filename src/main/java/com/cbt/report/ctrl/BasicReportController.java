...
public class BasicReportController {
     ...
     * @return
     */
    @RequestMapping(value = "/queryBalanceDetailsAll")
    @ResponseBody
    public EasyUiJsonResult queryBalanceDetailsAll(HttpServletRequest request) {
        ...
        List<Map<String, String>> monthLst = Util.genQueryDate(yearStr, monthStr);
        try {
            List<CustomerBalanceChangeBean> balanceChanges = basicReportService.queryBalanceDetailsAll(
                    monthLst.get(0).get("beginDate"), monthLst.get(0).get("endDate"), (stateNum - 1) * pageNum,
                    pageNum);
            int total = basicReportService.queryBalanceDetailsAllCount(monthLst.get(0).get("beginDate"),
                    monthLst.get(0).get("endDate"));
    ...
    @RequestMapping("/exportBalanceDetailsAllExcel")
    @ResponseBody
    public void exportBalanceDetailsAllExcel(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (!(yearStr == null || "".equals(yearStr.trim()) || "0".equals(yearStr.trim()) || "-1".equals(yearStr.trim())
            ...
            try {
                ...
                HSSFWorkbook wb = genBalanceDetailsAllExcel(balanceChanges, yearStr + "年" + monthStr + "客户余额变更全部详细");
                response.setContentType("application/vnd.ms-excel");
                response.setHeader("Content-disposition",
                        "attachment;filename=" + yearStr + "-" + monthStr + "-balanceChangeDetailsAll.xls");
                response.setCharacterEncoding("utf-8");
                ouputStream = response.getOutputStream();
                wb.write(ouputStream);
                ouputStream.flush();
    ...
    private HSSFWorkbook genBalanceDetailsAllExcel(List<CustomerBalanceChangeBean> balanceChanges, String title) {
        ...
        for (int i = 0; i < balanceChanges.size(); i++) {
            ...
            row.createCell(2).setCellValue(bc.getUserId());
            row.createCell(3).setCellValue(bc.getChangeBalance());
            row.createCell(4).setCellValue(DateFormatUtil.getWithSeconds(bc.getCreateTime()));
        }
        return wb;
    }
     ...
     * @param checkIsEmpty 为true时，把为数值类型的数据做为空的判断，为空的设置为'0'
     * @return
     */
