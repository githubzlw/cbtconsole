...
//	public void updateOrderChange2State(Map<String,Object> map);
   ...
   // @Select("SELECT pid FROM goods_list_search  WHERE Goods_state=1 LIMIT #{start},#{pagesize}")
    ...
    @Select("SELECT c.*,IFNULL(d.can_remaining,0) can_remaining FROM (SELECT a.final_weight,a.type_name as new_barcode ,b.sku,a.is_sold_flag,a.pid as goods_pid,b.enname as good_name,b.volume_weight,b.ali_price as goods_p_price,b.entype_new as entype,b.weight,b.max_price as goodsprice,concat(b.remotpath,b.custom_main_image) as  car_img " +
			"FROM (SELECT cb.final_weight,cb.is_sold_flag,cb.pid,st.type_name FROM custom_benchmark_sku cb,custom_benchmark_sku_type_name st WHERE cb.spec_id=st.spec_id AND cb.pid = #{pid})a, custom_benchmark_ready b WHERE a.pid=b.pid)c LEFT JOIN inventory d ON c.goods_pid=d.goods_pid LIMIT 1" )
