...
//			filename = StringUtils.getFileName(filename);
	...
	@ResponseBody
	protected Map<String,Object> barcodeMove(HttpServletRequest request, HttpServletResponse response){
		String admuserJson = Redis.hget(request.getSession().getId(), "admuser");
		Admuser adm = (Admuser) SerializeUtil.JsonToObj(admuserJson, Admuser.class);
		Map<String,Object> result = new HashMap<>();
		result.put("status", 200);
	...
	public ModelAndView lossList(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		try {
			...
			if(lossListCount > 0) {
				...
				int sum = inventoryLossList.stream().mapToInt(LossInventoryWrap::getChangeNumber).sum();
				result.addObject("lossListSum", sum);
			}
	 ...
	 * 库存报损下载
	 */
	@RequestMapping("/loss/download")
	public void lossListDownload(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		try {
			String strtype = request.getParameter("type");
			int type = StrUtils.isMatch(strtype, "(\\d+)") ? Integer.valueOf(strtype) : -1;
			String enddate = request.getParameter("enddate");
			...
			map.put("type",type );
			map.put("page",-1 );
			List<LossInventoryWrap> lossList = inventoryService.inventoryLossList(map);
			
			HSSFWorkbook wb = generalReportService.exportInventoryLoss(lossList);
			response.setContentType("application/vnd.ms-excel");
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			String filename = "inventory-loss-list-" + sdf.format(new Date())+".xls";
			response.setHeader("Content-disposition", "attachment;filename=" + filename);
			OutputStream ouputStream = response.getOutputStream();
			wb.write(ouputStream);
			ouputStream.flush();
