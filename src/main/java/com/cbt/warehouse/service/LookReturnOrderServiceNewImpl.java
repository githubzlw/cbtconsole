...
public class LookReturnOrderServiceNewImpl implements LookReturnOrderServiceNew {
	...
	public EasyUiJsonResult FindReturndisplay(String applyUser, String state,
		...
		list=this.lookReturnOrderServiceNewMapper.FindReturndisplay(nameString,state,a1688Shipno,optTimeStart,optTimeEnd,page,userOther,user,a1688order,pid,skuid);
		if (list.size()==0) {
            json.setRows(list);
            json.setTotal(0);
		return json;	
		}
		int total=this.lookReturnOrderServiceNewMapper.selectCount(nameString,state,a1688Shipno,optTimeStart,optTimeEnd,page,userOther,user,a1688order,pid,skuid);
