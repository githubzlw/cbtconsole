...
<script type="text/javascript">
	...
	var delreply=function(id){
		window.location = "/cbtconsole/customerServlet?action=delreply&className=GuestBookServlet&id="+id+"";
	};
	var check = function(){
		var uid = $("#userid").val();
		if(isNaN(uid)){
		...
		<table id="table"  width="100%" align="center" border="1px" style="font-size: 13px;" bordercolor="#8064A2" cellpadding="0" cellspacing="0" >
			...
			<tr>
			   <%--  <td>${i.index+1 }</td> --%>
				...
				<td>${cus.sendDate }</td>
		        <td>
		        <c:if test="${cus.recontent !=null }">
				<div  style="width:100%;height:120px;overflow: hidden;display: block;white-space: nowrap;text-overflow: ellipsis;">${cus.recontent }</div>
