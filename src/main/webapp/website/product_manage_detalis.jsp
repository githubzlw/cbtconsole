...
<script type="text/javascript">
	...
	function initXheditor() {
		$('#goods_content').xheditor({
			...
			upBtnText : "上传",
			upMultiple : 1,
			upImgUrl : "/cbtconsole/productEdit/uploads?pid=${goods.pid}",
	...
	function uploadEinfoNetImg(imgs, pid) {
		...
		$
				.ajax({
					type : 'POST',
					dataType : 'json',
					url : '/cbtconsole/productEdit/uploadEinfoNetImg',
					data : {
						"imgs" : imgs,
						"pid" : pid
					},
						...
						if (data.ok) {
							...
							if (urls.length > 0) {
								...
								for (var i = 0; i < urls.length; i++) {
									if (urls[i] != "") {
										srcs += '<img style="max-width:500px;height:auto;display:block;margin:10px auto;" src="'+urls[i]+'" />'
									}
								}
								if (srcs != "") {
									fmBody.append(srcs);
	...
	function doSaveDetalis(pid) {
		...
		$.ajax({
			type : 'POST',
			dataType : 'json',
			url : '/cbtconsole/productEdit/saveEditDetalis',
			data : {
				"pid" : pid,
				"enname" : enname,
				"weight" : weight,
	...
	function doQuery() {
		...
		if (pid == null || pid == "") {
			...
			window.location.href = "/cbtconsole/productEdit/detalisEdit?pid=" + pid;
		}
	}
	...
	function uploadTypePicture(pid) {
		...
		if (selectVal == '0') {
			...
			$
					.ajax({
						url : '/cbtconsole/productEdit/uploadByJs',
						type : 'POST',
						data : formData,
						contentType : false,
						processData : false,
			...
			$
					.ajax({
						type : 'POST',
						dataType : 'json',
						url : '/cbtconsole/productEdit/uploadTypeNetImg',
						data : {
							"imgs" : url,
							"pid" : pid,
							"check" : 1
						...
						success : function(data) {
							...
							if (data.ok) {
								//showMessage("执行成功，图片异步上传，请保存后刷新界面");
								$.messager.alert("提醒",
										"执行成功，图片异步上传，如若图片未显示，请保存后刷新界面，否则请重新上传",
										"info");
								var newImgUrls = data.data;
								var urls = newImgUrls.split(";");
								if (urls.length > 0) {
			...
			$.messager.confirm('提示', '是否使用速卖通详情?使用后编辑的详情数据将覆盖', function(rs) {
				if (rs) {
					...
					$.ajax({
						type : 'POST',
						dataType : 'text',
						url : '/cbtconsole/productEdit/useAliGoodsDetails',
						data : {
							"pid" : pid,
	...
	function publishGoods(pid){
		$.messager.confirm('提示', '是否已保存详情数据？是否上架该商品？上架中的商品统一走上架流程，非即时上架。上架中的商品将不再显示！', function(rs) {
			if (rs) {
				$.messager.progress({
					title : '正在处理数据',
					msg : '请等待...'
				...
				$.ajax({
					type : 'POST',
					dataType : 'text',
					url : '/cbtconsole/productEdit/publishGoods',
					data : {
						"pid" : pid
					},
					success : function(data) {
						...
						var json = eval('(' + data + ')');
						if (json.ok) {							
							$.messager.alert("提醒", "执行成功,请关闭当前页面！确认后，5秒后自动刷新页面！", "info",function(){
								setTimeout(function() {
									window.location.reload();
								}, 5000);
							});
...
<body>
	...
	<c:if test="${uid < 0}">
		<div>
			<p style="margin-left: 700px;font-size: 28px;color: red;margin-top: 200px;">该商品已在上架流程中</p>
		</div>
	</c:if>
		...
		<div class="allMain">
			<div class="s_top">
				...
				<c:if test="${goods.canEdit > 0}">
					...
					<c:if test="${goods.canEdit == uid}">
						<span class="s_btn" onclick="doSaveDetalis('${goods.pid}')">保存</span>
						<span class="s_btn" onclick="publishGoods('${goods.pid}')">上架该商品</span>
				...
				<div class="s_l">
					...
					<div class="s_box">
						<div class="detail_goods">
							<div class="goods_img">
								<div class="init_pic">
									<img src="${goods.showMainImage}" class="init_img"> <input
										type="hidden" id="goods_main_img"
										value="${goods.showMainImage}" />
	...
	<div class="s_r">
		...
		<div class="table_su">
			...
			</span> <br> <br> <a target="_blank" href="${goods.url}">1688原链接</a>
			<br> <a target="_blank" href="${goods.aliGoodsUrl}">速卖通原链接</a>
		</div>
		<div style="width: 80%; margin: 0 auto; margin-top: 150px">
	...
	<div class="s_bot">
		...
		<div class="bot_l">
			<div class="b_left">
				...
				<input type="hidden" id="goods_localpath" value="${localpath}"
					name="localpath"> <input type="hidden"
