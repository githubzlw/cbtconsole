...
<body>
	...
	<c:if test="${show > 0}">
		...
		<div>
			<h1 align="center">店铺商品公共图片展示(<span style="color:green">总数:${showTotal}</span>)</h1>
			<div>
			...
			<table border="1" cellpadding="0" cellspacing="0" align="center">
				...
				<c:forEach items="${newImgsList}" var="imgGd">
					<c:set var="count" value="${count+1 }" />
					<c:if test="${count == 1}">
						<tr>
					</c:if>
					<td>
						<div class="div_sty${imgGd.totalNum > 1 ? ' checkBg':''}">
							<label> 
							<c:if test="${imgGd.totalNum > 1}">
								<input class="check_sty isChoose" type="checkbox" checked="checked"
									value="${imgGd.pids}@${imgGd.imgUrl}" onclick="chooseBox(this)"/>
							</c:if>
							<c:if test="${imgGd.totalNum == 1}">
								<input class="check_sty" type="checkbox" value="${imgGd.pids}@${imgGd.imgUrl}" 
									onclick="chooseBox(this)"/>
							</c:if>
							&nbsp;<span
								style="color: ${imgGd.totalNum > 1 ? 'red':'#5dec5d'};">数量:[<b>${imgGd.totalNum}</b>]
							</span>
							</label> <br> <img class="img_sty" src="/cbtconsole/img/yuanfeihang/loaderTwo.gif" 
								data-original="${imgGd.imgUrl}" onclick="bigImg('${imgGd.imgUrl}')"/>
						</div>
					</td>
