...
function fn(va) {
	...
	if(admName!="Ling"  && admName!="Sales1" && admName!="Sales2"){
...
function getAllAdmuser(strRoletype,strAdmid,adminName) {
	...
	for (var i = 0; i < sellAdm.length; i++) {
		if(strRoletype==0 || adminName=="Ling"  || adminName=="Sales1" || adminName=="Sales2"){
			if(str=='') {
...
$(document).ready(function(){ 
	fn(1);
	fnGetStatistic();
	fnGetMessage(<%= user.getAdmName().equalsIgnoreCase("Sales1") ? 1 : uid%>); //获取各种消息数量
    uidTem = <%= user.getAdmName().equalsIgnoreCase("Sales1") ? 1 : uid%>;
}); 
