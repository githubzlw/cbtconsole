...
<form id="idForm" action="getExpressRecord.do" method="get">
	<div align="center" >
		<div><H1>扫描入库记录</H1></div>
		<div>                                                   
			快递号:<input class="querycss" style="width : 160px;" id="express_code" name="express_code" value="${oip.express_code}" type="text"/>
		...
		<div>
				...
				<table class="altrowstable">
					...
					<c:forEach items="${oip.recordlist }" var="obj" varStatus="s">
						<tr>
							<td>${s.index+1 }</td>
							<td>${obj.express_code }</td>
		...
		<div>
			<select class='classSelect' onchange="window.location=this.value" >
				<option selected="selected" value='getExpressRecord.do?pageNum=${oip.previousPage }&pageSize=${oip.pageSize}'>每页显示${oip.pageSize}条</option>
				
				<c:forEach begin="1" end="3" var="i">
					<c:if test="${i*50 != oip.pageSize}">
						<option value='getExpressRecord.do?pageNum=${oip.previousPage }&pageSize=${i*50 }'> 每页显示${i*50}条</option>
					</c:if>
				</c:forEach>
				
			</select>
			
			 <a href='getExpressRecord.do?pageNum=1&pageSize=${oip.pageSize }' class='className'>第一页</a>
			 <a href='getExpressRecord.do?pageNum=${oip.previousPage }&pageSize=${oip.pageSize }' class='className'>上一页</a>
			 <a href='getExpressRecord.do?pageNum=${oip.nextPage }&pageSize=${oip.pageSize }' class='className'>下一页</a>
			 <a href='getExpressRecord.do?pageNum=${oip.pageCount }&pageSize=${oip.pageSize }' class='className'>最后一页</a>
		</div>
		<div>
