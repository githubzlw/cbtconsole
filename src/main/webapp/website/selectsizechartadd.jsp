...
.top{overflow:hidden;}
.top select{float: left;line-height: 34px;height: 34px;margin-right: 15px;}
...
<body>
	...
	<div class="shopid_box">
		<div class="top">
			...
			</select> <select class="catoption_minlv">
			</select>
			<p class="shopid_p">
				<span class="submitButton" style="display: none;">下一页</span> 
				<span id="tipspan" style="display: none;font-size: 16px;color: red;">此类别已无可处理产品！</span>
			</p>
		</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		$(".catoption_lv1").change(function() {
			...
			if (opt == '内衣') {
				loadCategoryName('312');
			}
			if (opt == '运动服饰') {
				loadCategoryName('127380009');
			}
			if (opt == '女装') {
				loadCategoryName('10166');
			}
			if (opt == '男装') {
				loadCategoryName('10165');
			}
		});
	...
	function syncUpload(thispid,uploadtipObj,rowid, name, dataImg) {
		...
		var mydata = "method=syncUpload&imgFile=" + imgFile + "&imgName="
				+ name + "&rowid=" + rowid+"&pid="+thispid+"&add="+1;
		$.ajax({
			...
			success : function(data) {
				if(data.state>0){
					uploadtipObj.show();
					uploadtipObj.val("上传成功！");
				}else{
					uploadtipObj.show();
                    uploadtipObj.val("上传失败！");
				}
			}
		});
	}
	...
	function loadCategoryName(catid) {
		...
		if (catid == '311') {
			$(".catoption_lv1").val("童装");
		}
		if (catid == '312') {
			$(".catoption_lv1").val("内衣");
		}
		if (catid == '127380009') {
			$(".catoption_lv1").val("运动服饰");
		}
		if (catid == '10166') {
			$(".catoption_lv1").val("女装");
		}
		if (catid == '10165') {
			$(".catoption_lv1").val("男装");
		}
		$.ajax({
			type : "post",
			url : "/cbtconsole/order/loadCategoryName_add.do",
			data : {
				"catid" : catid
			},
			dataType : "text",
			...
			success : function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					var optionstr = "<option class='"+catid+"'>" + catName
							+ "</option>";
	...
	var rowidList = [];
	//加载 尺码表
	function getSizeChart(catid,nextpage_type) {
		var rowidListstr = "";
		//是点击下一页的话，需要更新前一页商品为已处理状态
		if(nextpage_type==1){
			rowidListstr = rowidList.join(',');
		}
		$.ajax({
		    type : "post",
		    url : "/cbtconsole/order/getSizeChart_add.do",
		    data : {
		        'catid' : catid,
		        'rowidListstr':rowidListstr
		    },
		    dataType : "text",
		    async : true,
		    success : function(data) {
			    ...
			    if (dataJson.length > 0) {
					...
					for (var i = 0; i < dataJson.length; i++) {
						...
						shopHtml = shopHtml + '<div class="imgs"><img src="'+ catimg.remotpath+catimg.custom_main_image + '" onclick="bigPic(this)"></div>';
					...
					$(".submitButton").show();
					$(".submitButton").attr("onclick","getSizeChart('" + catid + "',1)");
					$("#tipspan").hide();
				} else {
					$("#tipspan").show();
					$(".submitButton").hide();
