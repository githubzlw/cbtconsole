...
function fnSendEmail(){
	...
	$.post("/cbtconsole/WebsiteServlet",
  			{action:'sendEmail',className:'SendEmailServlet',emailInfo : emailInfo,email:emailaddress,copyEmail:copyEmail,orderNo:orderNo,userId:userId,title:titleinfo},
  			function(res){
				if(res>0){
					alert("发送成功");
					window.close();
...
<body>
	...
	<select id="type" onchange="fn(this.value)">
		<option value="1">额外运费减免</option>
		<option value="2">新钱入账(生成新订单)</option>
		<option value="4">Paypal审价中</option>
		<option value="3">其他沟通</option>
	</select>
	<br>标题:<input style="width: 260px;" type="text" lang="100" value="Your ImportExpress shipping discount!" id="titleinfo">
...
<a href="http://www.import-express.com" target="_blank"><img style="cursor: pointer" src="http://www.import-express.com/cbtconsole/img/logo.png"><img></a>
	<div style="border: 1px solid #BDBDBD;background-color: #EBEBEB;padding-left: 10px;width: 950px;"><div style="margin-bottom: 10px;font-weight: bolder;color: red;">PLEASE NOTE:</div>
	<div style="margin-bottom: 10px;font-size: 13px;">This email message was sent from a notification-only address that cannot accept incoming email. PLEASE DO NOT REPLY to this message. If you have any questions or concerns, Access your account 
	<a style="color: #0070C0" href="/cbtconsole<%=url %>" target="_blank">here</a>.</div></div>
<div style="font-weight: bolder;margin-bottom: 10px;">Dear <%=email%>,</div>
	<div>Order#:${param.orderno}</div>
	<div id="sendemailInfo">
		Due to the <span id="Reason"><input type="checkbox" id="reason1" value="delay">process delay/<input id="reason2" type="checkbox" value="partial shipment">partial shipment/<textarea id="reason3" rows="1" cols="89"></textarea></span>, we will decrease the freight cost for this order from <span id="freight">${param.currency}<input type="text" value="${param.actual_ffreight}" id="actual_ffreight"> to ${param.currency}<input type="text" value="${param.actual_ffreight1}" id="actual_ffreight1"></span>.
...
Thanks for your order, your paymant is currently under PAYPAL's varification and it may take few days for them to release the payment. Our purchase process will start as soon as we got PAYPAL's comfirmation. Thanks for your paitient and you may go to your <a href="/cbtconsole<%=url%>">accout</a> to check the order status. 
</div>
<br>
	<div style="font-weight: bolder;">Best regards,</div>
	<div style="font-weight: bolder;margin-bottom: 10px;">Import-Express.com</div>
