...
function insertTbGoodsSample(){
	...
	var discount = $("#discount").val();//整体折扣
	var ymx_discount = $("#ymx_discount").val();//亚马逊折扣
	if(ymx_discount==''){
		ymx_discount=0;
	}
	var discountprice = $("#discountprice").val();//商品价格总和
	if(discountprice==''){
		discountprice=0;
	}
	var minnum = $("#minnum").val();//最低数量
	var defaultnum = $("#defaultnum").val();//初始数量
...
//			}       
			...
			$("#gsd_amazongoosprice").val(((120-ymx_discount)*sPrice/100).toFixed(2));
			//商品的折扣价格=(原价*（100-折扣）)/100
		    $("#gsd_goodsprice").val(((100-discount)*sPrice/100).toFixed(2));
...
function JsPrice(sPrice){
	...
	$("#gsd_goodsprice").val((100-discount)*sPrice/100);
