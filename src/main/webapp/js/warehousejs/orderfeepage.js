...
//			</c:forEach>  "+objlist[i].orderno+"
			...
			//有值 查询第一个
			var orderidQuery = $("#orderidQuery").val();   
			var useridQuery = $("#useridQuery").val();
			$("#orderidQuery").val("");                   
			$("#useridQuery").val("");
			...
			if(orderidQuery ==''){
				...
				orderidQuery = objlist[0].orderno;
				useridQuery = objlist[0].userid;
			}      
			
			getOrderInfo(orderidQuery,useridQuery);  //订单信息
			                                  
			if( typeof(orderidQuery) != "undefined"){  
...
//订单包裹信息                      
function getShippingPackageById(orderid){
	$("#outddinfodiv").height(Number($("#outddinfodiv").height())-Number(height));
	$.ajax({
		type:"GET",                                                     
		url:"getSPById",
		dataType:"json",
		...
		success : function(data){  //返回受影响的行数
			//$("#packageAll").html();
			var temp = "";
	//		alert($("#packageAll").height());
			var packageHeight=0;
			for(var i=0; i<data.length; i++){                 
				packageHeight += 24;    
				temp += "包裹号"+(i+1)+":(<span id='bgh"+data[i].shipmentno+"'>"+data[i].shipmentno+"</span>)";          
				temp += "&nbsp;产品重量：<input  value='"+data[i].sweight+"' id='weight"+data[i].shipmentno+"' type='text'/>Kg&nbsp;&nbsp;&nbsp;&nbsp;";
				temp += "&nbsp;产品体积：<input  value='"+data[i].svolume+"' id='volumelwh"+data[i].shipmentno+"' type='text'/>CM<sup>3</sup>&nbsp;&nbsp;&nbsp;&nbsp;<br/>";
			}              
			height = packageHeight;
			$(".zldiv").height(packageHeight+55);                    
			$("#packageAll").html(temp);                    
		//	$("#outddinfodiv").height($("#outddinfodiv").height()+packageHeight);     
			$("#outddinfodiv").height(Number($("#outddinfodiv").height())+Number(packageHeight));
		}                
	});                                                                                             
}
...
//				private String sumPayment_amount; // 已支付产品费用
		//		alert(obj.sumProduct_cost+"-"+obj.sumPay_price_tow+"-"+obj.sumPayment_amount);
				...
				$("#s_payment_amount").html(obj.sumPayment_amount);  ////				&nbsp;已支付产品总金额:
...
function getOrderInfo(order_no,userid){
	//包裹信息                         
//	getShippingPackageById(order_no);      查询包裹这里先注释
...
//	alert($("#h_orderid").val());
	...
	var index=0;
	$("span[id^='bgh']").each(function(){
		                                              
		var weight = $("#weight"+$(this).html()).val();
		var volumelwh = $("#volumelwh"+$(this).html()).val();
		//alert($(this).html()+"--"+weight+"--"+volumelwh);
		
		var map = {}; 
  		map['shipmentno'] = $(this).html();
  		map['sweight'] = weight;                     
  		map['svolume'] = volumelwh;             
	...
	var mainMap={};  //主
	mainMap['listmap']=listmap;  
	mainMap['listmap2']=listmap2; //申报信息                
	mainMap['orderid']=$("#h_orderid").val(); 				//orderid
	mainMap['userid']=$("#h_userid").val();					//userid
	mainMap['courierNumber']=$("#courierNumber").val(); 	//快递单号
	mainMap['fpxCountryCode']=$("#fpxCountryCode").val(); 	//国家
