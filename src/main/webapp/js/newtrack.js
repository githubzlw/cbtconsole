 ...
 * @param repState
 */
function updategoodstatus(isok,goods_pid, orderid, goodid, itemid, taobaoprice, shipno,
 ...
 * @param repState
 */
function updateCheckStatus(isok,goods_pid, orderid, goodid, itemid, taobaoprice, shipno,
...
function allTrack(type) {
    ...
    // $.post(
    //         "/cbtconsole/WebsiteServlet?action=allTrack&className=ExpressTrackServlet",
    //         {
    //             type : type,
    //             shipno : shipno,
    //             barcode : barcode,
                ...
                if (res == "[]") {
                    ...
                    for (var i = 0; i < json.length; i++) {
                        ...
                        if(checked=="1"){
                            ...
                            str += '<p style="font-size:16px;font-weight:bold;" class="strcarype"><h3>已验货数量:<span id="'+json[i].orderid+'record_'+json[i].odid+'" style="font-size:30px;color:#ff0000;">'
                                +id_qty
                                + '</span></h3><h3>单件商品重量(kg):<input type="text" style="width: 70px;" id="'+json[i].orderid+'weight'+json[i].odid+'"></h3>';
                            ...
                            if (json[i].weight == undefined) {
                                ...
                                str += '<span name="save_weight">已保存的重量:' + json[i].weight + 'Kg&nbsp;</span><input type="button" value="将商品重量同步到产品库" style="margin-left:5px" onclick="saveWeight(\''+json[i].orderid+'\',\''+json[i].odid+'\',\''+json[i].goods_pid+'\')">';
                            }
                            str += '<br />';
                        ...
                        for (var i = 0; i < json.length; i++) {
                            ...
                            if (preorderid == json[i].orderid) {
                                ...
                                if(json[i].orderremark.length>0){
                                    ...
                                    var orderremark = json[i].orderremark;
                                    for (var j = 0; j < orderremark.length; j++) {
                                        barcode_info += '<p><<input type="checkbox" name="cha" value="货物偏差">货物偏差</p>';
                                        barcode_info +='<p>'+'备注人员：' + orderremark[j][2] + ',内容：' + orderremark[j][1] + '</p>';
                                    }
                                    barcode_info += '</div>';
                                }
                            ...
                            str += '<div>商品名称：<span id="name_'+json[i].odid+'">'
                                + json[i].goods_name
                                + '</span></div><div>';
                            str += '<div style="float:left;"><a href="https://www.import-express.com/goodsinfo/classic-vintage-heart-peach-heart-appearance-box-po-necklace-many-18888-122322005-1'+json[i].goods_pid+'.html" target="_blank">';/*if(json[i].img){
                                if (json[i].img[0].length > 5 && json[i].img != null && json[i].img != "undefined") {
                                    str += '<img width="300px" height="300px" src="'+json[i].img+'"/></a></div>';
                                } else{
                                    str += '<img  width = "300px" height="300px" src="'+img+'"/></a></div>';
                            ...
                            if(checked=="1"){
                            	console.log(1);
                                queryRecord(json[i].odid);
                                str += '<p style="font-size:16px;font-weight:bold;"><h3>已验货数量:<span id="'+json[i].orderid+'record_'+json[i].odid+'" style="font-size:30px;color:red;">'
                                    +id_qty
                                    + '</span></h3><h3>单件商品重量(kg):<input type="text" style="width: 70px;" id="\'+json[i].orderid+\'weight\'+json[i].odid+\'"></h3>';
                                    /*+ '<input type="button"  value="获取商品重量" onclick="getWeight(\''+json[i].orderid+'\',\''+json[i].odid+'\')">'*/
                                if (json[i].weight == undefined) {
...
function saveWeight(orderid,odid,pid){
    ...
    $.ajax({
        ...
        data:{orderid:orderid,odid:odid,weight:weight,pid:pid},
        dataType:"json",
        success:function(data){
            if(Number(data) == 1){
...
function saveClothingData(){
    ...
    $.ajax({
        ...
        data:{orderid:orderid,goodsid:goodsid
            ,catid:catid,y_wg:y_wg,y_bmxj:y_bmxj,y_wgxj:y_wgxj,y_ll:y_ll,y_xt:y_xt,y_zg:y_zg,y_qw:y_qw,y_bz:y_bz,jks_value:jks_value,jk_remark:jk_remark,xwb_value:xwb_value,
            xw_remark:xw_remark,yww_value:yww_value,yw_remark:yw_remark,twh_value:twh_value,tw_remark:tw_remark,xcs_value:xcs_value,xc_remark:xc_remark,yzl_value:yzl_value,yc_remark:yc_remark
        },
        dataType:"json",
        success:function(data){
            if(Number(data)>0){
...
function saveJewelryData(){
    ...
    $.ajax({
        ...
        data:{orderid:orderid,goodsid:goodsid,catid:catid,s_wz:s_wz,s_ks:s_ks,s_ys:s_ys,s_bm:s_bm,s_wgxj:s_wgxj,s_lk:s_lk,s_ds:s_ds,s_cz:s_cz,s_bz:s_bz
        },
        dataType:"json",
        success:function(data){
            if(Number(data)>0){
...
function saveElectronicData(){
    ...
    $.ajax({
        ...
        data:{orderid:orderid,goodsid:goodsid,catid:catid,d_wg:d_wg,d_ks:d_ks,d_ys:d_ys,d_wz:d_wz,d_wg1:d_wg1,d_wg2:d_wg2,
            d_ds:d_ds,d_nc:d_nc,d_rl:d_rl,d_pm:d_pm,d_fbv:d_fbv,d_xs:d_xs,d_ct:d_ct,d_sms:d_sms,d_bz:d_bz
        },
        dataType:"json",
        success:function(data){
            if(Number(data)>0){
